          ...
          <>
            {homepageSettings.facebookUrl?.trim() ? (
              <a
                ...
                className={`${baseBtn} bottom-24 right-4 md:right-6`}
                aria-label="فيسبوك دعم الدكتور (يمين)"
                title="فيسبوك — دعم الدكتور"
              >
                <span className={`${circle} bg-[#1877F2]`}>{fbIcon}</span>
                <span className={label}>دعم الدكتور (Facebook)</span>
              </a>
            ) : null}
            ...
            {homepageSettings.whatsappUrl?.trim() ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`${baseBtn} bottom-6 right-4 md:right-6`}
                aria-label="واتساب دعم الدكتور (يمين)"
                title="واتساب — دعم الدكتور"
              >
                <span className={`${circle} bg-[#25D366]`}>{waIcon}</span>
                <span className={label}>دعم الدكتور (WhatsApp)</span>
              </a>
            ) : null}

            {homepageSettings.teamFacebookUrl?.trim() ? (
              <a
                href={homepageSettings.teamFacebookUrl.trim()}
                target="_blank"
                rel="noopener noreferrer"
                className={`${baseBtn} bottom-24 left-4 md:left-6`}
                aria-label="فيسبوك دعم الفريق (يسار)"
                title="فيسبوك — دعم الفريق"
              >
                <span className={`${circle} bg-[#1877F2]`}>{fbIcon}</span>
                <span className={label}>دعم الفريق (Facebook)</span>
              </a>
            ) : null}

            {homepageSettings.teamWhatsappUrl?.trim() ? (
              <a
                href={homepageSettings.teamWhatsappUrl.trim()}
                target="_blank"
                rel="noopener noreferrer"
                className={`${baseBtn} bottom-6 left-4 md:left-6`}
                aria-label="واتساب دعم الفريق (يسار)"
                title="واتساب — دعم الفريق"
              >
                <span className={`${circle} bg-[#25D366]`}>{waIcon}</span>
                <span className={label}>دعم الفريق (WhatsApp)</span>
              </a>
            ) : null}
          </>
        );
